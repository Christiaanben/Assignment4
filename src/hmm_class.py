...
class HMM(object):
    ...
    def _score(signal, seq, trans, dists):
        ...
        ----------
        signal : (d,n) ndarray
            Signal for whipassch the optimal state sequence is to be calculated.
            d is the dimension of each observation (number of features)
            n is the number of observations 
...
stateLength = []
        ...
        for state in states:
            ...
            if state[stateLooker] != True:
                stateLooker += 1
                stateLooker = stateLooker % (states.shape[1])
                stateLength.append(currentStateLength)
                currentStateLength = 0
    ...
    def _updatecovs(self, states):
        ...
        '''
        covs  = np.zeros((states.shape[1],self.data.shape[0],self.data.shape[0]))
        means = np.zeros((states.shape[1],self.data.shape[0]))

        data = []
        for i in range(states.shape[1]):
            data.append([])
        for i in range(len(states)):
            data[np.argmax(states[i])].append(self.data[0][i])
    ...
    def _em(self, trans, states):
        ...
        Parameters
        ----------
        trans : (n_statvies+1,n_states+1) ndarray
            The left-to-right transition probability table.  The rightmost column contains probability
            of transitioning to final state, and the last row the initial state's
            transition probabilities.   Note that all the rows need to add to 1. 
        ...
        '''
        covs, means = self._updatecovs(
            states)  # Initialize the covariances and means using the initial state allocation
